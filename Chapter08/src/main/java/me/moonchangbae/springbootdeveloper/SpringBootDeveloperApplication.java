...
/*
    ...
    학습 목표 : 로그인 / 회원 가입 기능을 구현
        ...
        /login 요청이 들어올 때,
            -> UserViewController 가 해당 요청에 대한 분기 처리를 하고 WebSecurityConfig 에 실행한 보안 관련
                내용들을 실행
            -> UserDetailsService 를 실행하면 요청을 성공했을 떄
            ...
            1. 스프링 시큐리티(Spring Security)
                ...
                3) 필터 기반으로 동작하는 스프링 시큐리티
                    스프링 시큐리티의 풀 로그인 과정을 설정하는 건(코드로 작성하는 건) 그렇게 어렵지 않음.
                    내부적으로 아까 보여드린 그림처럼 꽤 복잡한 과정을 거치게 됩니다.
