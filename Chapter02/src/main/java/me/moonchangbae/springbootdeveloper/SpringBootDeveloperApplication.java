...
/*
    New project 생성시 주의점 :
        ...
        2. DLS -> Groovy 설정
        3. name = ArtifactId
        4. build.gradle 설정을 복사하는데, 복사 후에 sync 를 꼭꼭 눌러줘야함.
            -> SpringBootDeveloperApplication 에서 @SpringBootApplication 에너테이션에 빨간줄 가있다면
                새로고침 안했을 확률이 높음.
        5. resources 내에 static 내에 index.html 이라고 하는데, 해당 폴더명의 경우 대부분의 개발자들이 합의한 형태
...
/*
    1. Spring vs. SpringBoot
        ...
        SpringBoot의 등장
            ...
            특징 :
                1) 톰캣, 네티, 언더토우 같은 웹 애플리케이션 서버가 내장돼있어 따로 설치하지
                    않아도 독립적으로 실행 가능
            ...
            차이점 :
                ...
                2) 내장 WAS 유무 -> 스프링 애플리케이션은 일반적으로 톰캣과 같은 WAS 에서 배포
                    ...
                    하지만 스프링 부트는 자체적으로 WAS 를 가지고 있어 JAR 파일만 만들면 빌드로
    ...
    2. 스프링 컨셉
        ...
        3) 빈과 스프링 컨테이너
            3)-1. 스프링 컨테이너 : 빈이 생성되고 소멸되기 까지의 생명주기를 관리함
                또한 @Autowired 와 같은 애너테이션을 사용해 빈을 주입받을 수 있게 DI 를 지원함.
            ...
            3)-2. 빈 : 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                ...
                ex) 클래스를 빈으로 등록하는 방법 예제
                    ...
                    public class MyBean{}
                        ...
                        소문자로 바꾸어서 관리함. 즉, 클래스 MyBean 의 빈 이름은 myBean 이 됨.
    ...
    3. 스프링 부트 3 둘러보기
        첫 번째 스프링 부트 3 예제 만들기
            ...
            스프링 부트 스타터 살펴보기
                ...
                스타터의 명령 규칙
                    spring-boot-starter-web : Spring MVC 를 사용해 RESTful 웹 서비스를 개발할 때 필요한 의존성 모음
