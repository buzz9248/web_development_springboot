...
@EnableJpaAuditing          // created_at, updated_at 자동 업데이트 해주는 애너테이션
...
/*
    1. 사전 지식 : 타임리프(Thymeleaf)
        타임리프는 템플릿 엔진입니다. 다만 템플릿 엔진은 HTML 과 템플릿 엔진을 위한 문법을 섞어서 사용.
            즉 이제는 HTML 지식이 약간 필요함.

            -> 저희는 다음 과정에서 HTML 을 배우게 되므로 아주 간단한 화면 구성을 위해서 코드를 작성할 예정.
        ...
        * 템플릿 엔진 : 스프링 서버에서 데이터를 받아 우리가 보는 웹 페이지, 즉 HTML 상에서
        ...
        템플릿 엔진 개념 잡기
            ...
            h1 태그에서는 ${이름}이 텍스트 어트리뷰트(속성)으로 할당되어 있습니다.
            p 태그도 비슷한 상황입니다.
            이상이 템플릿 문법의 예시인데,
            이렇게 해두면 서버에서 이름, 나이라는 key 로 데이터를 템플릿 엔진에 넘겨주고,
            ...
            값이 달라지면 그때그떄 화면에 반영하니 동적인 웹 페이지를 만들 수 있게 되는 것입니다.
            ...
            *{...} - 선택한 변수의 표현식. th:object 에서 선택한 객체에 접근
            ...
            th:unless - 조건이 false 일 때만 표시   - th:unless'${person.age}>=20'
