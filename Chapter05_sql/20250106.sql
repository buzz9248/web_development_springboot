...
-- users 와 orders 를 하나로 결합하여 출력합니다.(단, 주문 정보가 있는 회원의 정보만 출력)
...
-- 이상의 SQL문에 대한 해석
-- 기존의 FROM 다음에는 테이블 명 하나만 작성되었지만, 이제는 JOIN 연산을 위한 추가 문법이 적용됐음.
-- 회원 정보와 주문 정보를 하나로 결합하기 위해 users와 orders 를 INNER JOIN 으로 묶고, '후속조건'으로 "주문정보가 있는 회원의 정보만 출력하기 위해" u.id = o.user_id 를 적용함.
	
-- 여기서 여러분들에게 연습문제시에만 설명했던 부분을 명확하게 하고 넘어가야 합니다.

-- users PK인 id는 회원id에 해당
-- orders PK인 id는 주문 id에 해당하고, 2번째 컬럼인 user_id는 orders에서는 PK는 아니지만 JOIN을 수행할 때 users와 합치는 조건이 됩니다.
	

-- 여러 테이블을 하나의 FROM에서 다룰 때에는 별칭을 사용 가능(ALIAS와는 다름 -> 컬럼명을 지정.)
-- FROM users u 로 작성했을 때 이후에는 u 만 썼을 경우 users 테이블을 의미하게 됨.
...
-- user_id 가 일부 기준이 되어야 합니다.

-- 왜냐하면 users 내에 있는 id는 1부터 끝까지 있음
-- users 테이블에는 회원 id 가 id 컬럼에 기록되어있고, orders 테이블에는 회원 id가 user_id로
...
-- FROM 에서 JOIN이 정렬된 후에 단일 테이블에 명령을 내리는 것처럼 쿼리를 작성 가능
...
-- (nullable = false 로 Entity 클래스에서 지정했습니다)

-- cf) 키 값은 테이블 내에서 고유한 값을 가지므로 한 테이블에서 개수를 계산할 때 중복되진 않는다.
-- 하지만 여러 테이블을 조인하면 '키 값도 중복될 수 있다.' 예를 들어, 회원 아이디가 7인 사람이
-- 세 번 주문했다면 회원 정보(users)와 주문 정보(orders) 를 겨랍한 결과에는 u.id = 7인 
-- 행에 3개 있을 것이다. 이 경우 '한 번 이라도 주문한 회원 수'를 중복없이 구하려면 회원 id를
-- 중복 제거한 뒤에 회원 수를 count 할 필요가 있다.

-- key의 구분
-- 1. 기본 키(Primary Key) : 하나의 테이블에서 가지는 고유한 값
-- 2. 외래 키(Foreign Key) : 다른 테이블에서 가지는 기존 테이블의 값

-- FK는 다른 테이블의 고유한 키 값인 PK를 참조한다.(orders 에서 o.id 가 FK에 해당,
...
-- 이를 JOIN 시켰을 때, u.id로 ORDER BY 하더라도 합친 테이블 상에서 제 1칼럼(즉 PK 컬럼)에
-- 동일 id가 여러 번 출력될 수 있습니다.

-- JOIN의 종류
-- 1. INNER JOIN
--	: 두 테이블의 키 값이 일치하는 행의 정보만 가지고 온다. -> 집합에서 '교집합'에 해당.
...
-- 즉, 두 테이블의 교집합과 교집합에 속하지 않은 왼쪽 차집합을 불러옵니다.
-- 왼쪽 테이블의 값을 모두 불러오기 때문에 LEFT JOIN / LEFT OUTER JOIN 이라고 합니다.

-- LEFT JOIN 과 INNER JOIN은 함께 실무에서 자주 쓰입니다. 데이터를 결합하는 경우 대부분
...
-- 두 텐이블에 동일한 컬럼이 있을 때(대부분의 경우 PK 역할을 하는 id들은 다 있으니까요),
-- 어떤 것을 지정했는지 명확히 하기 위해서 입니다.
-- 이를 활용해서 SELECT에서도 * 대신에 표기할 컬럼을 지정할 수 있는데,
...
-- 단, 주문정보가 없는 회원의 주문 정보도 모두 출력하고, 다음 컬럼을 출력하자. u.id, u.username, u.phone, o.user_id, o.id, od.order_id, od.product_id
...
-- users와 orders를 하나로 결합하여 출력해보자(단, 회원정보가 없는 주문정보도 출력)
SELECT *
	FROM users u
...
-- 즉 여기에서의 예제 쿼리의 결과 값은 INNER JOIN과 동일하게 됩니다.
...
-- 결합으로 봤을 때 orders는 users 테이블에 종속되어 있으므로 null 값이 출력될 수 없습니다.
...
SELECT *
	FROM users u 
	CROSS JOIN orders o	-- 모든 가능한 행 조합에서 -> CROSS JOIN을 유추
	ORDER BY u.id;

-- CROSS JOIN - 두 테이블 간의 집합을 조합해 만들 수 있는 모든 경우의 수를 생성하는 방식으로,
	-- 카테시안 제곰(Cartesian Product)을 의미함.
-- u.id를 기준으로 정렬했습니다.(얼마나 많은 조합이 나오는지 시각적으로 보여드릴 수 있도록 추가함)

-- u.id 와 o.user_id를 연결하는 등의 조건 없이 두 테이블의 모든 행을 합쳐서 만들 수 있는 모든 경우의 수를 생성한 것입니다.

-- 즉, 10 행의 테이블과 20행의 테이블을 CROSS JOIN하면 200 행이 됩니다.
-- 해당 경우 CROSS JOIN은 모든 경우의 수를 구하므로 ON 조건을 굳이 설정하지 않습니다.

-- JOIN 명령어는 근본적으로 두 테이블의 행을 서로 조합하는 과정에 해당하는데, ON 조건을 활용하면
-- 전체 경우의 수에서 어떤 행만 가져올 수 있는지 정할 수 있음.

-- 실제 환경에서는 CROSS JOIN을 제한하는 편. 컴퓨터에 많은 연산을 요구하는데, 실질적으로는 NULL 값만 출력되는 경우의 수가 많아 쓸모는 없기 때문이다.

-- 연습 문제
-- 1. users와 staff를 참고하여 회원 중 직원인 사람의 회원 id, 이메일, 거주 도시, 거주 국가, 성, 이름을 한 화면에 출력
...
-- 2. staff와 orders를 참고하여 직원 아이디가 3번, 5번인 직원의 담당 주문을 출력
-- (단, 직원 아이디, 직원 성, 주문 아이디, 주문일자 만 출력)
...
-- 4. orders와 orderdetails, products를 참고하여 회원 아이디별 주문 금액의 총합을 정상 가격과 할인 가격 기준으로 각각 구하세요.
-- (단, 정상 가격 주문 금액의 총합 기준으로 내림차순으로 정렬하세요.)

SELECT o.user_id, sum(p.price * od.quantity) AS sumPrice, SUM(p.discount_price * od.quantity) AS sumDiscountPrice 
	FROM orders o 
	LEFT OUTER JOIN orderdetails od
		ON o.id = od.order_id
...
-- 정리 1. 기본 형식
-- FROM [테이블명 1] a(별칭) [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명 2] b(별칭)
...
-- FROM [테이블명 1] a(별칭) [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명 2] b(별칭)
-- ON [JOIN 조건1]
-- [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명 3] c(별칭)
-- ON [JOIN 조건2]

-- FROM 내에서 JOIN을 여러 번 중첩 사용 가능. 앞의 JOIN의 ON 절 뒤에 새로운 JOIN 명령어를
-- 작성하면 제 3의 테이블과 결합 가능 -> 순서대로 적용한다는 점이 중요합니다.
-- 횟수 제한은 없지만, 테이블 크기에 따라 많은 연산이 필요할 수 있어 사전에 필요한 연산인지 점검할 필요가 있습니다.


-- 정리 3. INNER JOIN
-- FROM [테이블명 1] a INNER JOIN [테이블명 2] b
-- ON a.key = b.key

-- INNER JOIN은 각 테이블의 키 값이 '일치하는 행만' 가져옵니다. 집합으로 보면 교집합
-- 가장 기본적인 JOIN문으로, 간혹 INNER을 생략하고 JOIN만 쓰기도 합니다.(default JOIN 구문).
-- 그런데 실무에서는 가독성을 위해 INNER 명시.

-- 정리 4. OUTER JOIN(LEFT/RIGHT)
-- FROM [테이블명 1] a LEFT/RIGHT/FULL JOIN [테이블명 2] b 
-- ON a.key = b.key

-- OUTER JOIN은 LEFT OUTER JOIN, RIGHT OUTER JOIN, FULL OUTER JOIN을 포괄하는 용어
-- OUTER 생략 가능

-- LEFT OUTER JOIN : 왼쪽 테이블의 모든 데이터 값을 결과에 포함(보통 자주 쓰임)
-- RIGHT OUTER JOIN : 오른쪽 테이블의 모든 데이터 값을 결과에 포함(잘 안씀)
-- FULL OUTER JOIN : 왼쪽과 오른쪽에 있는 테이블의 모든 값을 결과에 포함
-- 			LEFT + RIGHT JOIN의 결과값이 중복없이 결합된 형태.
--			DB에 따라서 지원 하지 않는 경우도 많음.

-- 정리 5. CROSS JOIN
-- FROM [테이블명 1] a CROSS JOIN [테이블명 2] b 
-- CROSS JOIN은 두 테이블을 결합했을 때 각 테이블의 행으로 만들 수 있는 모든 조합을 결과값으로 도축하는 연산 -> 카테시안 곱이라고 함. -> 쓸 때 주의
