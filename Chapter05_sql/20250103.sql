-- users에서 country 별 회원 수 구하기 쿼리문
...
-- users 에서 country가 Korea인 회원 중에서 마케팅 수신에 동의한 회원 수를 구해 출력할 것
-- 표시 컬럼 uniqueUserCnt 컬럼으로 구성돼있습니다.
...
-- 표시 컬럼 country, uniqueUserCnt 두 컬럼으로 구성돼있습니다.
...
-- 			국가별로는 오름차순, uniqueUserCnt 기준으로는 내림차순으로 정렬
-- SELECT 절 - FROM 테이블 명 - GROUP BY 절 - ORDER BY 절 : 순서

GROUP BY 에 두 개 이상의 기준 컬럼을 추가하면 데이터가 여러 그룹으로 나뉨.
아르헨티나를 기준으로 했을 때 마케팅 수신 동의 여부가 0인 회원 수와 마케팅 수신 여부가 1인 회원 수를 기준으로 나뉘어져 있음을 알 수 있음.
...
-- SUBSTR(컬럼명, 시작값, 글자개수)
-- users 에서 월별(e.g. 2013-10) 가입 회원 수를 출력할 것.
...
-- 1. orderdetail 에서 order_id 별 주문 수량 quantity의 총합을 출력
...
-- 2. orders에서 staff_id 별, user_id 별로 주문 건수를(COUNT(*)) 출력할 것
	-- 단, staff_id 기준 오름차순하고, 주문 건수 별로 내림차순
...
SELECT SUBSTR(order_date, 1, 7) , COUNT(DISTINCT user_id) AS countUser
	FROM play_sql.orders o 
...
-- HAVING
-- GROUP BY 를 이용해서 데이터를 그룹화하고, 해당 그룹별로 집계 연산을 수행하여
...
-- 언제나 WHERE을 쓰는 것이 용이하지 않다는 것 부터 짚고 넘어가서 HAVING 학습을 할 예정

-- users 에서 country가 Korea, USA, France 인 회원 숫자를 도출할 것
SELECT country, COUNT(DISTINCT id)
	FROM play_sql.users u
	WHERE country IN ("Korea", "USA", "France")
	GROUP BY country;

WHERE 을 통해서 원하는 국가만 먼저 필터링하고, GROUP BY 했습니다.
...
-- users에서 회원 수가 8명 이상인 country 별 회원수 출력(회원 수 기준 내림차순)
...
2. 즉 GROUP BY 를 사용한 집계 값을 필터링할 때는 -> HAVING 사용.

-- orders 에서 담당 직원별 주문 건수와 주문 회원 수를 계산하고, 주문 건수가 10건 이상이면서
	-- 주문 회원 수가 40명 이하인 데이터만 출력(단, 주문 건수 기준으로 내림차순 정렬. 단, staff_id, users_id, id 컬럼 이용)

SELECT staff_id, COUNT(DISTINCT id), COUNT(DISTINCT user_id) 
	FROM play_sql.orders o 
	GROUP BY staff_id 
	HAVING COUNT(DISTINCT id) >= 10
		AND COUNT(DISTINCT user_id) <=40
	ORDER BY COUNT(DISTINCT id) DESC;
...
1. 컴퓨터는 가장 먼저 FROM 을 읽어 데이터가 저장된 위치에 접근하고, 테이블의 존재 유무를 마치고
테이블을 확인하는 작업을 실행하고,
...
6. ORDER BY 를 통해 특정 컬럼 및 연산 결과를 통한 오름 차순 및 내림 차순으로 나열함.


-- 연습문제
-- 1. orders 에서 회원별 주문 건수 추출하라(단 주문 건수가 7건 이상인 회원의 정보만 추출, 주문 건수 기준으로 내림차순 정렬, user_id와 주문 아이디(id) 컬럼 활용 할 것 )


SELECT user_id, id, COUNT(DISTINCT id) 
	FROM play_sql.orders o 
	GROUP BY user_id
...
-- 2. users 에서 country 별 city 수와 국가별 회원(id) 수를 추출. 단, 도시 수가 5개 이상이고 회원 수가 3명 이상인 정보만 추출하고, 도시 수, 회원 수 기준으로 모두 내림차순할 것

SELECT country,COUNT(DISTINCT city), COUNT(DISTINCT id) 
	FROM play_sql.users u 
	GROUP BY country 
	HAVING COUNT(DISTINCT id) >= 3
		AND COUNT(DISTINCT city) >= 5
	ORDER BY COUNT(DISTINCT city) DESC, COUNT(DISTINCT id) DESC; 




-- 3. users에서 USA, Brazil, Korea, Argentina, Mexico 에 거주 중인 회원 수를 국가별로 추출(단, 회원 수가 5명 이상인 국가만 추출하고, 회원 수 기준으로 내림차순)
	-- (hint : 얘는 where 도 있고 having 도 있다.)
