...
-- 해당 이유는 : UNION은 결합하는 두 결과 집합에 대한 '중복 제거 기능'이 포함되어 있습니다.
...
-- users에서 country가 Korea인 회원정보만 추출하고, Mexico인 회원정보만 추출해서 결합해보자. (단, 컬럼은 id, phone, city, country 만 출력하고 country 기준 알파벳 순 정렬)
...
-- 두 결과 집합을 UNION ALL을 사용해 하나로 결합.(단, 최종 결과는 최신순으로 정렬)
...
-- YYYY-MM-DD 형식으로 지정되어 있다면, 문자열 비교 결과와 실제 날짜 비교 결과가 동일하게 적용함.
-- 그래서 MM-DD-YYYY 형식으로 되어있다면, 오류가 발생할 가능성이 있음.
...
-- 2. users에서 USA에 거주 중이면서 마케팅 수신 동의(1)한 회원 정보와 France에 거주 중이면서
-- 마케팅 수신에 동의하지 않은(0) 회원 정보를 SELECT로 각각 추출하고, 두 결과 집합을 UNION ALL을
-- 사용해 하나로 결합.(단, 최종 결과는 id, phone, country, city, is_markeing_agree
-- 컬럼 추출하고, 거주 국가 기준으로 알파벳 역순으로 추출.)

(
SELECT 
	id, phone, country, city, is_marketing_agree
FROM 
	users
WHERE 
	country = "USA"
...
(SELECT *
	FROM orderdetails od LEFT OUTER JOIN products p 
	ON od.product_id = p.id)
UNION 
(SELECT *
	FROM orderdetails od RIGHT OUTER JOIN products p
	ON od.product_id = p.id);
...
-- products에서 name, price 모두 불러오고, '평균 정상 가격을 새로운 컬럼'으로 각 행마다 출력.

SELECT 
	name, price, (SELECT AVG(price) FROM products)  
FROM 
	products;
-- SELECT AVG(price) FROM products; 를 하는 경우 전체 price / 행의 개수로 나눈 데이터가 단 하나이므로, name, price, SELECT AVG(price) FROM products; 로 작성하면 1행짜리만 도출됨.
...
SELECT 
	name, price, 38.5455 AS avgPrc  
FROM 
	products;

-- 특정된 단일 결과값을 각 행에 적용을 하고 싶다면 이상과 같은 하드코딩이 가능
...
-- 스칼라 서브 쿼리를 작성할 때 '단일 값'이 반환되도록 작성해야 한다는 점에 유의.
-- 만약 2개 이상의 집계 값을 기존 테이블에 추가하여 출력하고 싶다면, 스칼라 서브 쿼리로 따로
-- 나누어 작성해야 합니다.

-- users에서 city별 회원 수를 카운트하고, 회원 수가 3명 이상인 도시명과 회원 수를 출력
-- 단, 회원수를 기준으로 내림차순
SELECT
	city, COUNT(DISTINCT id) 
FROM 
	users u
GROUP BY 
	city;
...
FROM 
	(
		SELECT 
			city, COUNT(DISTINCT id) AS cntUser
		FROM
			users u 
		GROUP BY 
			city
...
-- 1번 쿼리는 서브 쿼리 없이 작성한 예시
-- 2번 쿼리는 서브 쿼리 포함된 해설 -> 해당 문제에서 눈여겨 볼 것은 굳이 따지자면
-- 여기서는 서브쿼리를 작성하는데 있어서 스칼라 서브 쿼리 형태로 작성하는 것이 중요하다.
-- 해당 문제에서 검증 결과 서브 쿼리 자체가 필요하지는 않습니다.
-- a 이 부분도 비표준 SQL이라서 일단은 적어만 두겠습니다.


-- orders와 staff를 활용해 last_name이 Kyle이나 Scott인 직원의 담당 주문을 출력.
-- (단, 서브쿼리 형태를 활용하자)
...
FROM 
	staff
WHERE 
	last_name IN ("Kyle", "Scott");				
-- 조건절에 쓰일 경우에 scalar 서브가 아니었다는 점에 주목


-- 이상의 코드는 staff 테이블에서 id 값이 3, 5를 도출했습니다.
...
FROM 
	orders
WHERE 
	(staff_id, user_id) IN (			-- 필터링 대상 컬럼 개수 = 2
		SELECT 							
			id, user_id 				-- 서브 쿼리 컬럼 개수 = 2
		FROM 
			staff
		WHERE 
			last_name IN ("Kyle", "Scott")
...
-- SELECT 절에서는 '단일 집계 값'을 신규 컬럼으로 추가하기 위해 서브 쿼리를 사용.
-- 여러 개의 컬럼을 추가하고 싶을 때는 서브 쿼리를 여러 개 작성하면 됩니다.
-- 특징 : SELECT의 서브 쿼리는 메인 쿼리의 FROM에서 사용된 테이블이 아닌 테이블도 사용 가능하기
...
-- 특징 : 
-- FROM 에서 서브 쿼리를 적절히 활용하면 적은 연산으로 같은 결과를 도출 가능.
-- 단, RDBS 기준 테이블 검색을 빠르게 할 수 있느 *인덱스 개념이 있는데 서브 쿼리를 활용하면 인덱스를 사용하지 못하는
-- 경우가 있으므로 주의해야 함.
...
-- 특징 : IN 연산자의 경우에 다중 컬럼 비교를 할 때는 서브 쿼리에서 ㅊ추출하는 컬럼의 개수와
-- WHERE에 작성하는 필터링 대상 컬럼 개수가 일치해야함. -> 이 때 필터링 대상 컬럼이 2개 이상이면
-- ()로 묶어서 작성해야 함.
...
-- 4. 서브 쿼리로 필터링(WHERE + 서브 쿼리)
-- 5. 같은 행동 반복 대상 추출(LEFT OUTER JOIN)

-- 1. users에서 created_at 컬럼 활용하여 연도별 가입 회원 수를 추출.

SELECT 
	SUBSTR(created_at, 1, 4), COUNT(DISTINCT id) 
FROM 
	users
WHERE
...
-- 2. users에서 country, city, is_auth 컬럼을 활용하여 국가별, 도시별로 본인 인증한 회원 수를 추출.
 
-- 1)
SELECT 
	country, city, is_auth, COUNT(DISTINCT id) 
FROM 
	users
WHERE 
	is_auth = 1
...
SELECT 
	country, city, SUM(is_auth)			-- is_auth가 1이면 본인인증한거니까 is_auth 들의 합이 10이라면 본인인증한 회원 숫자가 10이라는 뜻
FROM
	users
GROUP BY 
	country, city 
