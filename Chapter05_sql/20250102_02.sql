...
-- 문자열을 원하는 만큼 잘라내거나 대/소문자로 변경하는 등의 기능을 수행하는 '일반 함수'
-- 등으로 나눌 수 있다.
...
-- 예를 들어, '국가별 회원 수를 구하라' 혹은 '일별 매출 계산하라' 같은
-- 그룹화 기준을 지정해서 원하는 계산을 수행할 수 있음.

-- GROUP BY 로 계산한 결과를 필터링하는 HAVING 명령어가 있음.
-- GROUP BY 를 적용해서 '국가별 회원 수를 구하고' 이후에 '그리고 회원 수가 10명 이상인'
-- '국가만 도출하라'는 등 집계 함수로 계산한 결과 중 조건에 맞ㅂ는 데이터만 필요한 경우에는
-- WHERE을 쓰는 것이 아니라 'HAVING' 을 사용한다

-- 집계 함수 예시
-- 예제 : users 에서 모든 행 수를 세어봅니다 : COUNT
SELECT *
	FROM play_sql.users u;

SELECT COUNT(*)						-- COUNT(컬럼명, 사실 *) : 집계함수에 해당
	FROM play_sql.users u;			-- 대상 테이블 전체 행의 개수를 세는 역할
...
-- country의 데이터값의 종류를 계산하고 싶다면 어떡해야 할까?
...
-- products 에서 최고가를 구하시오(price 컬럼 이용)
SELECT MAX(price)
	FROM play_sql.products p;

-- products 에서 전체 데이터 가격의 합 구하시오(price 컬럼 이용)
SELECT SUM(price)
	FROM play_sql.products p;

-- products 에서 price의 평균(단, 결과값의 컬럼명을 avgPrice로 변경)
...
ROUND(컬럼명, 소수점자리수) : 소수점자리를 지정한 자릿수까지 반올림하여 변환
SUBSTR(컬럼명, 시작위치, 가져올문자개수) : 문자열을 지정한 시작 위치부터 지정한 문자 개수만큼 반환
...
이상의 이유로 집계 함수는 SELECT 에서만 사용 가능
...
-- 예제 : users 에서 country 가 Korea 인 회원 수를 출력
SELECT COUNT(id) 
	FROM play_sql.users u 
	WHERE country = "Korea";
...
SELECT COUNT(DISTINCT country)
	FROM play_sql.users u;
-- 를 확인해보면 COUNT(DISTINCT country)의 결과값이 17 이므로
-- 모든 국가를 대상으로 회원 수를 확인해보기 위해서는 국가별로 즉, 총 17번의 
-- SQL 문을 작성해야 합니다.
-- 국가 수는 적지도 않고, 실제 환경에서는 더 많은 데이터를 다루는데, 조건을 일일이 적용하기에는 비효율적임
-- 즉, WHERE절에서 country = 의 값을 매번 바꾸는 방식은 사용하지 않습니다.

-- 다음 ㅣ간에 저희는 GROUP BY를 통해 어떻게 묶을지 학습하도록 하겠습니다.
