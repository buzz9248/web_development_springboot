...
/*
        ...
        스프링 부트에서의 계층
            ...
            3) 퍼시스턴스(Persistence)
                ...
                DAO(추후 수업 예정) 객체를 사용할 '수도' 있음
                    * DAO - 데이터베이스 계층과 상호작용하기 위한 객체라고 이해하면 편합니다.
    ...
    스프링 부트 프로젝트 디렉토리 구성
        : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따로 -
            ...
            test : 프로젝트의 소스 코드를 테스트할 목적의 코드나 리소스 파일이 들어있음
                (원래는 자동생성되는데 어제 제가 만들었을 때 안만들어지는 걸로 보아 설정 따라 다른듯합니다)
            ...
            main 디렉토리 구성하기
                ...
                01 단계 - HTML 과 같은 뷰 관련 파일(chapter07에서 수업 예정)을 templates
                    resources > templates
    ...
    2. 스프링 부트 3 프로젝트 발전시키기
        ...
        지금은 의존성을 추가한 다음에 -> 프레젠테이션 계층, 비지니스 계층, 퍼시스턴스 계층 순서로
            ...
            implementation : 프로젝트 코드가 컴파일 시점과 런타임에 모두 해당 라이브러리를 필요로 할 때 지정
        ...
        지시 사항
            프레젠테이션, 서비스, 퍼시스턴스 계층 만들기
                ...
                3. 퍼시스턴스 계층 코드 작성 -> 같은 위치에 Member.java 생성
                4. 매핑 작업에는 '인터페이스' 파일이 필요. MemberRepository 인터페이스 같은 위치에 생성.
