...
/*
    ...
    사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
        ...
        토큰 기반 인증 :
            * 토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            Chapter08 에서는 스프링 시큐리티를 이용하여 세션 기반 인증을 사용해 사용자마다
            사용자의 정보를 담은 세션을 생성하고 인증했음 -> 세션 기반 인증
            ...
            토큰 기반 인증의 특징
                ...
                JWT(Json Web Token) -> json 형태로 주고받음
                    발급받은 JWT 를 이용해 인증을 하려면 HTTP 요청 헤더 중에 Authorization 키 값에
                        ...
                        * 토큰 타입과 해싱 알고리즘 지정 예시
                        ...
                        2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)
                            ...
                            구성 :
                                ...
                                nbf : 토큰의 활성 날짜와 비슷한 개념. Not Before 를 의미
                                    이 날짜가 지나기 전까지는 토큰이 처리되지 않음.
                               iat : 토큰이 발급된 시간. issued at 을 의미
                               jti : 토큰의 고유 식별자로서 주로 일회용 토크네 사용
                        ...
                        URI 로 네이밍된 https://ahngeunsu.com/jwt_claims/is_admin : 공개 클레임
                        ...
                        3) 서명 : 해당 토큰이 조작되었거나 변경되지 않았음을 확인하는 용도로 사용.
                            ...
                            토큰 유효 기간
                                ...
                                토큰 유효 기간이 너무 짧다면 사용자가 불편함.
                                    -> 리프레시 토큰의 개념이 발생
                                    * 리프레시 토큰 : 엑세스 토큰과 별개로 사용자를 인증하기 위한 용도가 아니라
                                        엑세스 토큰이 만료되었을 때 엑세스 토큰을 발급하기 위해 사용.
    ...
    2. JWT 서비스 구현
        ...
        토큰 제공자를 추가할겁니다.
            jwt 를 사용해서 JWT 를 생성하고 유효한 토큰인지 검증하는 클래스 추가할 예정
