...
class BlogApiControllerTest {
    ...
    /*
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            ...
            형식에 맞게 변환하는 과정이 필요. 이 작업을 직렬화/역직렬화 라고 함.

                1) 직렬화 Java 시스템 내부에서 사용되는 객체를 외부에서 사용하도록
                    변환하는 과정을 뜻하는데,
                    예를 들어 title 은 "제목", content 는 "내용" 이라는 값이 들어가 있는
                    개체가 있다고 가정했을 때,
                    자바 상에서는
                    ...
                    public class Article {                  클래스, 필드, 생성자 정의
                        ...
                        형태로 정리됩니다 -> 포스트맨에서 확인함.

                글 생성 API 를 테스트 하는 코드를 작성
                Given - 블로그 글 추가에 필요한 요청 객체를 생성
                When - 블로그 글 추가 API 에 요청을 보냅니다. 요청 타입은 JSON 이며, given 절에서
    ...
    public void addArticle() throws Exception {
        ...
        // 객체를 JSON 형태로 직렬화 하는 과정
        final String requestBody = objectMapper.writeValueAsString(userRequest);

        // when
        // 설정한 내용을 바탕으로 요청을 전ㅅ농
        ...
        List<Article> articles = blogRepository.findAll();      // repository 에서 글(객체)를 가져와 List 에 담음.

        assertThat(articles.size()).isEqualTo(1);    // list 내의 element 개수 확인해서 그게 1이면 true

        // list 인 articles 를 확인해서 가장 첫번째(0번) 객체를 꺼내서 title 을 확인했더니 String "title" 인지 확인.
        ...
        /*
            ...
            그 이후에 MockMvc 를 사용해 HTTP 메서드, URL, 요청본문, 요청 타입 등을 설정 뒤 테스트 요청 보냈습니다.
            contentType() 메서드 : JSON / XML 등 다양한 타입 중 하나를 선택할 수 있는데, 저희는 JSON 사용.
            assertThat() 메서드 : 글의 개수가 1인지,
                Article 객체의 field 인 title 이 "title" 인지,
                Article 객체의 field 인 content 가 "content" 인지 확인.
    ...
    /*
        삭제 구현
            BlogService 로 가서 delete() 메서드 추가 -> blogRepository 를 기준으로
    ...
    public void deleteArticle() throws Exception {
        ...
        mockMvc.perform(delete(url, savedArticle.getId())) // findArticle() 처럼 하나의 객체를 대상으로 할 때는 argument 가 url 만 있는게 아닙니다.
                .andExpect(status().isOk());

        // then     - 검증단계에서 블로그 글 목록을 전부 다 가지고 올겁니다. -> size() 체크 했더니 0이거나/ .isEmpty() 메서드 활용
        // 전부 다 가지고 오는 단계
