...
class MemberRepositoryTest {
    ...
    void getAllMembers() {
        ...
        // memberRepository 는 상속받은 메서드 명을 그대로 가져가니까
        // 우리가 직접 정의하는 영어가 아니라도 익숙해져야 함.
    ...
    /*
        ...
        member 라는 테이블의 모든 컬럼을 조회하는데, id = 2 인 객체의 값들을 보여달라.
    ...
    void getMemberById() {
        // when
        Member member = memberRepository.findById(2L).get(); // 2L 에서 L 의 의미 : Long

        // then
        assertThat(member.getName()).isEqualTo("B");
    }
    /*
        이제는 id 가 아니라 name 으로 찾기로 해보겠습니다.
        id = 기본키
        name 의 경우에는 컬럼이 있을 수도 없을 수도 있기 때문에 JPA 에서 기본으로 name 을 찾아주는
        메서드가 없음.

        필수 컬럼이 아닌 것을 조건으로 검색을 하기 위해서는 MemberRepository.java 에서
        메서드를 추가할 필요가 있습니다.
    ...
    /*
        이상과 같이 MemberRepository.java 에 method 를 정의하는 것을 '쿼리 메서드' 라고 함
        일반적인 경우 JPA 가 정해준 메서드 이름 규칙을 따르면 쿼리문을 특별히 구현하지 않아도
        메서드처럼 사용할 수 있음(즉 자바만으로 컨트롤이 가능하다는 의미)

        이상의 메서드를 SQL 로 작성할 경우
    ...
    void saveMember() {
        ...
        //when
        memberRepository.save(member);  // save 가 자동완성 됐다는 점에도 주목해야 함
    ...
    @Test
    void saveMembers() {
        // given - 다수의 객체를 저장할 예정이므로 collections 를 사용 -> List
        List<Member> members = List.of(new Member(2L, "문창배"), new Member(3L, "문창배2"));

        // when
        memberRepository.saveAll(members);      // members 라는 List 를 전체 저장
    ...
    /*
        ...
        JPA 에서 데이터를 수정하는 방식은 추후에 SQL 에서 배우게 될 트랜잭션
        내에서 데이터를 수행해야 합니다.
     ...
     */
        ...
        @Test
        void update() {
            //given         id 2인 객체를 가지고 와서 member 라는 객체명에 저장 / 현재 name = "B"
