...
/*
    ...
    1. 사전 지식 : API 와 REST API
        ...
        식당으로 알아보는 API
            여러분이 손님인데, 식당 들어감 -> 점원에게 요리를 주문(주방으로 들어가서 요리를 주문하는게 아니다)
            그리고 점원은 주방에 가서 요리를 만들어달라고 요청합니다.
                  요청        요청
            손님 <----> 점원 <----> 주방
                  응답        응답

        의 형태를 띄고 있는데, 여기서 손님은 클라이언트(client), 주방에서 일하는 요리사를 서버(server) 라고 생각.
        그리고 중간에 있는 점원을 API -> '매개체'라고 한 점 기억.

        우리는 웹 사이트의 주소를 입력해서 '구글 메인 화면을 보여줘' 라고 요청하면, API 는 요청을 받아서 서버에게 전달.
        그러면 서버는 API 가 준 요청을 처리해 결과물을 구성해서 이것을 다시 API 에게 전달하고, API 는 결과물을
        브라우저에 보내주면 화면을 볼 수 있게 됩니다.
            이처럼 API 는 클라이언트의 요청을 서버에 잘 전달하고, 서버의 결과물을 클라이언트에게 잘 돌려주는 역할을 함.


        그렇다면 REST API 란?

        웹의 장점을 최대한 활용하는 형태라고 알려진 REST API.
            Representational State Transfer 을 줄인 표현으로, 자원을 이름으로 구분해 자원의 상태를 주고 받는 API 방식
        ...
        REST 하게 디자인한 API 를 보고 RESTful API 라고 부르기도 한다.

        REST API 사용하는 방법
            규칙 1. URL 에는 동사를 쓰지 않고, 자원을 표시한다.
                * 자원 : 가져오는 데이터를 의미. 예를 들어 학생 중에 id 가 1인 학생의 정보를 가져오는 URL은
                1) 예시 : /students/1
                2) 예시  : /get-student?student_id=1
            과 같은 방식으로 설계할 수 있는데,

            이 중 더 REST API 에 맞는 형식은 1) 예시에 해당함.
            2) 예시의 경우 자원이 아닌 다른 표현을 섞어 사용했기 때문
            (get).
            동사를 사용해서 생기게 되는 추후의 문제점 예시 -> 데이터를 요청하는 URL 을 설계할 때
            ...
            규칙 2. 동사는 HTTP 메서드로
                HTTP 메서드 : 서버에 요청을 하는 방법을 나누는 것. -> POST, GET, PUT, DELETE
                    만들고, 읽고, 업데이트하고, 삭제한다 Create / Read / Update / Delete 라고 해서 CRUD 라고 함.
                예를 들어 블로그에 글을 쓰는 설계를 한다고 가정했을 떄

                    1) id 가 1인 블로그 글을 '조회'하는 API : GET/articles/1
                    2) 블로그 글을 '추가'하는 API           : POST/articles          -> 추가가 완료되기 전까지 DB상 id가 없음.
